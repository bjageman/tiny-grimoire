...
describe('Silent Disconnect Heartbeat', () => {
  beforeEach(() => {
    localStorage.clear();
    sessionStorage.clear();
    activeSubscriptions.length = 0;
  ...
  it('drops the connected icon after a player vanishes without a deliberate leave', async () => {
    vi.useFakeTimers();
    try {
      window.location.hash = '#/standard';
      const storyteller = render(<StandardSetup theme="dark" toggleTheme={vi.fn()} />);
      ...
      // Simulate a silent disconnect (network drop / app killed): the client goes away
      // with no player_leave and sends no more heartbeats.
      joinPage.unmount();

      // Past the heartbeat timeout plus a stale-check tick — no deliberate leave was sent.
      await act(async () => {
        await vi.advanceTimersByTimeAsync(70_000);
      });

      // The player's seat stays, but the connected icon drops.
      expect(storyteller.container.querySelector('#edit-player-button-p-alice')).not.toBeNull();
      expect(storyteller.container.querySelector('#edit-player-button-p-alice svg.lucide-wifi')).toBeNull();
  ...
  it('keeps the connected icon while heartbeats keep arriving', async () => {
    vi.useFakeTimers();
    try {
      window.location.hash = '#/standard';
      const storyteller = render(<StandardSetup theme="dark" toggleTheme={vi.fn()} />);
      ...
      // Same elapsed time as the sibling test, but Alice's client stays mounted this time,
      // so it keeps sending heartbeats every 20s throughout.
      await act(async () => {
        await vi.advanceTimersByTimeAsync(70_000);
      });
